...
ui <- dashboardPage(
    ...
    dashboardBody(
        ...
        fluidRow(
            ...
            column(width = 6,  box(title = "Stock Graph", width = NULL, solidHeader = FALSE, plotOutput("stockPlot", height = "275px")))
        ),
        fluidRow(
            br(),
            box(width=6,
...
#       
        minPrice <- min(stockPrice)
        maxPrice <- max(stockPrice)
        meanPrice <- round(mean(stockPrice),2)
        
        totReturn <- (stockPrice[length(stockPrice)]/stockPrice[1] -1)
        totReturn <- paste0(round(totReturn*100,2), "%")
        
        minYear <- as.numeric(format(min(stockData()$Date), format="%Y"))
        maxYear <- as.numeric(format(max(stockData()$Date), format="%Y"))

        avgReturn <- (stockPrice[length(stockPrice)]/stockPrice[1] -1)
        avgReturn <- paste0(round(avgReturn/(maxYear - minYear)*100,2),"%")
        
       data.frame(
           Title = c("Minimum Price:",
                    "Maximum Price: ",
                    "Mean: ",
                    "Total Return: ",
                    "Average annual Return: "),
           Value = as.character(c(minPrice,
                                  maxPrice,
                                  meanPrice,
                                  totReturn,
                                  avgReturn
